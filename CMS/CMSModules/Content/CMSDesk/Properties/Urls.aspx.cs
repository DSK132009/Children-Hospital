...
public partial class CMSModules_Content_CMSDesk_Properties_Urls : CMSPropertiesPage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (txtSlug.TextBox.Text.Length > 50)
        {
            ShowError("URL Slug cannot be longer than 50 characters");
            SetUrlTextbox(false);
            return;
        }
        ...
        if (new PageUrlPathSlugUpdater(Node).TryUpdate(txtSlug.TextBox.Text, out var collisions))
        {
            ...
            //Even though nothing is changing, this will trigger the Update event to check for Node Alias synchronization
            Node.Update();
        }
        else
        {
